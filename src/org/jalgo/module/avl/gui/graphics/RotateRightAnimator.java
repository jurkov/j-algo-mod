 ...
 */
public class RotateRightAnimator
extends Animator {
	...
	public RotateRightAnimator(PaintArea pa, Graphics2D offG, Node pivot) {
		this.pa = pa;
		this.offG = offG;
		this.pivot = pivot;
	...
	public void run() {
		...
		for (int y = 0; y <= Y_DIST_NODES[Settings.getDisplayMode()]; y++) {
			...
			offG.fillRect(boundingRect.x, boundingRect.y, boundingRect.width,
				boundingRect.height);
			pa.drawTree(pivot.getLeftChild().getLeftChild(), 0, -y);
			pa.drawTree(pivot.getRightChild(), 0, y);
			...
			if (pivot.getLeftChild().getRightChild() != null) {
				...
				int y_l = (int)Math.sqrt(
					...
					Math.pow((pa.getXFor(pivot.getLeftChild()) -
						pa.getXFor(pivot.getLeftChild().getRightChild())) -
						(int)xd_y,
					2));
				int y_l_hoehe =
					pa.getYFor(pivot.getLeftChild().getRightChild()) - y_l;
				...
				pa.drawTree(
					pivot.getLeftChild().getRightChild().getLeftChild(), 0, 0);
				pa.drawTree(
					pivot.getLeftChild().getRightChild().getRightChild(), 0, 0);

				pa.drawLine(
					pa.getXFor(pivot.getLeftChild().getRightChild()),
					pa.getYFor(pivot.getLeftChild().getRightChild()),
					pa.getXFor(pivot.getLeftChild()) - (int)xd_y,
					y_l_hoehe,
					pivot);

				/*
				 * pa.drawLine(pa.getXFor(pivot.getLeftChild().getRightChild()),
				 * pa.getYFor(pivot.getLeftChild().getRightChild()),
				 * pa.getXFor(pivot.getLeftChild())-xd2,
				 * pa.getYFor(pivot.getLeftChild()),pivot);
				 */
				pa.drawNode(pa.getXFor(pivot.getLeftChild().getRightChild()),
					pa.getYFor(pivot.getLeftChild().getRightChild()), pivot
					.getLeftChild().getRightChild());
			}

			pa.drawLine(
				pa.getXFor(pivot.getLeftChild()),
				pa.getYFor(pivot.getLeftChild()) - y,
				pa.getXFor(pivot),
				pa.getYFor(pivot) + y,
				pivot);
			pa.drawNode(
				pa.getXFor(pivot.getLeftChild()),
				pa.getYFor(pivot.getLeftChild()) - y, pivot.getLeftChild());
